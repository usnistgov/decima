...
import gov.nist.decima.core.schematron.SchematronHandler;
...
public abstract class AbstractDerivedRequirementParsingHandler implements SchematronHandler {
  private static final Logger log
      = LogManager.getLogger(AbstractDerivedRequirementParsingHandler.class);

  private final Map<String, List<SchematronAssertionEntry>> patternIdToAssertionsMap
      = new HashMap<>();
  ...
  public boolean handlePattern(Element pattern) {
    ...
    if (currentPatternId == null) {
      ...
      log.error(
          "Multiple schematron pattern elements have the same id attribute '" + currentPatternId
              + "'. This will prevent the evaluator from determining if a derived requirement is "
              + TestState.NOT_APPLICABLE);
    }
    return true;
  ...
  public boolean handleRule(Element rule) {
    ...
    if (currentRuleId == null) {
      log.error("A schematron rule element is missing an id attribute."
          + " This will prevent the evaluator from determining if an assertion is "
          + TestState.TESTED);
  ...
  public void handleReport(Element reportElement) {
    handleElement(SchematronAssertionEntry.AssertionType.REPORT,
        getDerivedRequirement(reportElement));
  }
  ...
  public void handleAssert(Element assertElement) {
    handleElement(SchematronAssertionEntry.AssertionType.ASSERT,
        getDerivedRequirement(assertElement));
  }

  private void handleElement(SchematronAssertionEntry.AssertionType type,

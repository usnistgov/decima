...
public class JDOM2ObjectModel extends TreeModel implements ExternalObjectModel {
  ...
  /**
   * Get the name of a characteristic class, which, if it can be loaded, indicates that the
   * supporting libraries for this object model implementation are available on the classpath
   *
  ...
  /**
   * Get a converter that converts a sequence of XPath nodes to this model's representation of a
   * node list.
   *
   * @param node
  ...
  /**
   * Convert a Java object to an XPath value. If the supplied object is recognized as a
   * representation of a value using this object model, the object model should convert the value to
   * an XPath value and return this as the result. If not, it should return null. If the object is
   * recognized but cannot be converted, an exception should be thrown
   *
   * @param object
  ...
  /**
   * Wrap a document node in the external object model in a document wrapper that implements the
   * Saxon DocumentInfo interface
   *
   * @param node

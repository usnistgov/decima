...
/**
 * This interface must be implemented by any third-party object model that can be wrapped with a
 * wrapper that implements the Saxon Object Model (the NodeInfo interface). This implementation of
 * the interface supports wrapping of JDOM Documents.
 */
...
public class JDOM2ObjectModel extends TreeModel implements ExternalObjectModel {
  ...
  /**
   * Get the name of a characteristic class, which, if it can be loaded, indicates that the supporting
   * libraries for this object model implementation are available on the classpath
   *
   * @return by convention (but not necessarily) the class that implements a document node in the
   *         relevant external model
   */
  ...
  /**
   * Get the URI of the external object model as used in the JAXP factory interfaces for obtaining an
  ...
  /**
   * Get a converter that converts a sequence of XPath nodes to this model's representation of a node
   * list.
   *
   * @param node
   ...
   *         return a PJConverter that takes a list of XPath nodes (represented as NodeInfo objects)
   *         and returns a collection of nodes in this object model
   */
  ...
  /**
   * Test whether this object model recognizes a given class as representing a node in that object
   * model. This method will generally be called at compile time.
   *
   * @param nodeClass
   ...
   * does, return a Receiver that builds an instance of this data model from a sequence of events. If
   * the Result is not recognised, return null.
   */
  ...
  /**
   * Test whether this object model recognizes a particular kind of JAXP Source object, and if it
   * does, send the contents of the document to a supplied Receiver, and return true. Otherwise,
   * return false.
   */
   ...
   * and return this as the result. If not, it should return null. If the object is recognized but
   * cannot be converted, an exception should be thrown
   *
   * @param object
   *          the object to be converted . If this is not a JDOM node, the method returns null
   * @param config
   ...
   * that the supplied class might be a List, in which case the method should inspect the contents of
   * the Value to see whether they belong to this object model.
   *
   * @param value
   *          the XPath value to be converted
   * @param targetClass
   *          the Java class required for the result of the conversion
  ...
  /**
   * Wrap a document node in the external object model in a document wrapper that implements the Saxon
   * DocumentInfo interface
   *
   * @param node
   *          a node (any node) in the third party document
   * @param config
   ...
   *
   * @param document
   *          the document wrapper, as a DocumentInfo object
   * @param node

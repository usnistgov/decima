...
public class JDOM2NodeWrapper extends AbstractNodeWrapper implements SiblingCountingNode {
  ...
  /**
   * Determine the relative position of this node and another node, in document order. The other node
   * will always be in the same document.
   *
   * @param other
  ...
  /**
   * Get the string values of all the nodes in a list, concatenating the values into a supplied string
   * buffer
   *
   * @param list
  ...
  /**
   * Get the prefix part of the name of this node. This is the name before the ":" if any. (Note, this
   * method isn't required as part of the NodeInfo interface.)
   *
   ...
   * position of the underlying JDOM nodes; thus the sibling position for the text node is that of the
   * first JDOM node to which it relates, and the numbering of subsequent XPath nodes is not
   ...
   *          overwrite the contents of this array, to avoid the cost of allocating a new array on the
   *          heap.
   ...
   *         array is a sequence of namespace codes, whose meaning may be interpreted by reference to
   *         the name pool. The top half word of each namespace code represents the prefix, the bottom
   *         half represents the URI. If the bottom half is zero, then this is a namespace
   *         undeclaration rather than a declaration. The XML namespace is never included in the list.
   *         If the supplied array is larger than required, then the first unused entry will be set to
   *         -1.
   *         <p>

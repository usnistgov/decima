...
public class JDOM2NodeWrapper extends AbstractNodeWrapper implements SiblingCountingNode {
    ...
    protected Object node; // the JDOM node to which this XPath node is mapped;
    // or a List of
    // adjacent text nodes
    ...
    /**
     * This constructor is protected: nodes should be created using the wrap factory method on the
     * DocumentWrapper class
     *
     * @param node
     *            The JDOM node to be wrapped
     * @param parent
    ...
    /**
     * Factory method to wrap a JDOM node with a wrapper that implements the Saxon NodeInfo
     * interface.
     *
     * @param node
     *            The JDOM node
     ...
     *            The wrapper for the Document containing this node
     * @param parent
     *            The wrapper for the parent of the JDOM node
     * @param index
    ...
    /**
     * Determine the relative position of this node and another node, in document order. The other
     * node will always be in the same document.
     *
     * @param other
     *            The other node, whose position is to be compared with this node
     * @return -1 if this node precedes the other node, +1 if it follows the other node, or 0 if
     *         they are the same node. (In this case, isSameNode() will always return true, and the
     *         two nodes will produce the same result for generateId())
     */
    ...
    /**
     * Get the value of the item as a CharSequence. This is in some cases more efficient than the
     * version of the method that returns a String.
     */
    ...
    /**
     * Get the string values of all the nodes in a list, concatenating the values into a supplied
     * string buffer
     *
     * @param list
     *            the list containing the nodes
     * @param sb
     *            the StringBuffer to contain the result
     */
    ...
    /**
     * Get the prefix part of the name of this node. This is the name before the ":" if any. (Note,
     * this method isn't required as part of the NodeInfo interface.)
     *
    ...
    /**
     * Get the URI part of the name of this node. This is the URI corresponding to the prefix, or
     * the URI of the default namespace if appropriate.
     *
     * @return The URI of the namespace of this node. For an unnamed node, or for a node with an
     *         empty prefix, return an empty string.
     */
     ...
     *
     * @return The display name of this node. For a node with no name, return an empty string.
     */
    ...
    /**
     * Get the index position of this node among its siblings (starting from 0) In the case of a
     * text node that maps to several adjacent siblings in the JDOM, the numbering actually refers
     * to the position of the underlying JDOM nodes; thus the sibling position for the text node is
     * that of the first JDOM node to which it relates, and the numbering of subsequent XPath nodes
     * is not necessarily consecutive.
     */
    ...
    protected AxisIterator iterateDescendants(NodeTest nodeTest, boolean includeSelf) {
        ...
        if (nodeTest.getUType() == UType.ELEMENT) {
            ...
            descendants = ((Parent) node).getDescendants(Filters.element());
        } else {
    ...
    private static class DescendantWrappingIterator extends NodeWrappingAxisIterator<Content> {

        public DescendantWrappingIterator(Iterator<Content> descendantIterator,
                NodeWrappingFunction<Content, NodeInfo> wrappingFunction) {
        ...
        @Override
        public boolean isIgnorable(Content node) {
     ...
     * <p>
     * Note: the result is equivalent to getEnumeration(AxisInfo.CHILD,
     * AnyNodeTest.getInstance()).hasNext()
     */
    ...
    /**
     * Get a character string that uniquely identifies this node. Note: a.isSameNode(b) if and only
     ...
     *
     * @param buffer
     *            If this is non-null, and the result array fits in this buffer, then the result may
     *            overwrite the contents of this array, to avoid the cost of allocating a new array
     *            on the heap.
     * @return An array of integers representing the namespace declarations and undeclarations
     *         present on this element. For a node other than an element, return null. Otherwise,
     *         the returned array is a sequence of namespace codes, whose meaning may be interpreted
     *         by reference to the name pool. The top half word of each namespace code represents
     *         the prefix, the bottom half represents the URI. If the bottom half is zero, then this
     *         is a namespace undeclaration rather than a declaration. The XML namespace is never
     *         included in the list. If the supplied array is larger than required, then the first
    ...
    /**
     * The class ChildEnumeration handles not only the child axis, but also the following-sibling
     * and preceding-sibling axes. It can also iterate the children of the start node in reverse
     * order, something that is needed to support the preceding and preceding-or-ancestor axes (the
     * latter being used by xsl:number)
     */
    ...
    private final class ChildEnumeration extends AxisIteratorImpl {
        ...
        private int ix = 0;
        private boolean downwards; // iterate children of start node (not
        // siblings)
        private boolean forwards; // iterate in document order (not reverse
        // order)

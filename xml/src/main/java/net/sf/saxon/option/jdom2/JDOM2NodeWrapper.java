...
public class JDOM2NodeWrapper extends AbstractNodeWrapper implements SiblingCountingNode {
   ...
   *         are the same node. (In this case, isSameNode() will always return true, and the two
   *         nodes will produce the same result for generateId())
   */
   ...
   * URI of the default namespace if appropriate.
   *
   * @return The URI of the namespace of this node. For an unnamed node, or for a node with an empty
  ...
  /**
   * Get the display name of this node. For elements and attributes this is [prefix:]localname. For
   * unnamed nodes, it is an empty string.
   *
   ...
   * node that maps to several adjacent siblings in the JDOM, the numbering actually refers to the
   * position of the underlying JDOM nodes; thus the sibling position for the text node is that of
   * the first JDOM node to which it relates, and the numbering of subsequent XPath nodes is not
   ...
   *
   * @param uri
   *          the namespace URI of the attribute name. Supply the empty string for an attribute that
   *          is in no namespace
   * @param local
   *          the local part of the attribute name.
   * @return the attribute value if it exists, or null if it does not exist. Always returns null if
  ...
  /**
   * Get a character string that uniquely identifies this node. Note: a.isSameNode(b) if and only if
   * generateId(a)==generateId(b)
   *
   * @param buffer
   ...
   *          overwrite the contents of this array, to avoid the cost of allocating a new array on
   *          the heap.
   * @return An array of integers representing the namespace declarations and undeclarations present
   *         on this element. For a node other than an element, return null. Otherwise, the returned
   *         array is a sequence of namespace codes, whose meaning may be interpreted by reference
   *         to the name pool. The top half word of each namespace code represents the prefix, the
   *         bottom half represents the URI. If the bottom half is zero, then this is a namespace
   *         undeclaration rather than a declaration. The XML namespace is never included in the
   *         list. If the supplied array is larger than required, then the first unused entry will
   *         be set to -1.
   *         <p>
  ...
  /**
   * The class ChildEnumeration handles not only the child axis, but also the following-sibling and
   * preceding-sibling axes. It can also iterate the children of the start node in reverse order,
   * something that is needed to support the preceding and preceding-or-ancestor axes (the latter
   * being used by xsl:number)
   */

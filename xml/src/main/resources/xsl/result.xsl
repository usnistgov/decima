...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="target-name" select="res:assessment-results/@subject"/>
    ...
    <xsl:template match="/">
        <html>
            <head>
                ...
                <link href="{$bootstrap-path}/css/bootstrap.min.css" rel="stylesheet"/>
                <style>
                    .row-eq-height {
                    display: -webkit-box;
                    display: -webkit-flex;
            ...
            <body>
                <script type="text/javascript" src="{$bootstrap-path}/js/jquery-3.1.0.min.js"/>
                <script type="text/javascript" src="{$bootstrap-path}/js/bootstrap.min.js"/>
                <!-- https://github.com/chartjs/Chart.js/releases/tag/v2.2.2 -->
                <script type="text/javascript" src="{$bootstrap-path}/js/Chart.min.js"/>
                <script type="text/javascript" src="{$bootstrap-path}/js/clipboard.min.js"/>
                <script type="text/javascript" src="{$bootstrap-path}/js/jquery.filtertable.min.js"/>
                <script>
...
/*
					function filterAction(e) {
						console.log("Target index: "+$(e.target).index());
						var colindex = $(e.target).index()+1;
						$(e.target)
							.parents('table')
							.find($('tbody > tr > td:nth-child('+colindex+')'))
							.each(function() {
								console.log("Target value: "+$(e.target).val());
								console.log("this index: "+$(this).index());
								console.log("this text: "+$(this).text());
								var regex = new RegExp($(e.target).val(), 'i');
								var parent = $(this).parent();
								console.log("TR class before: "+$(parent).attr('class'));
								if (regex.test($(this).text())) {
									if ($(parent).hasClass('filtered')) {
										$(parent).removeClass('filtered');
									}
								} else {
									if (true != $(parent).hasClass('filtered')) {
										$(parent).addClass('filtered');
									}
								}
								console.log("TR class after: "+$(parent).attr('class'));
							});
					}
					*/
                </script>
                                ...
                                <div class="panel panel-default">
                                    ...
                                    <div class="panel-body">
                                        <dl class="dl-horizontal dl-narrow">
                                            <dt>Target:</dt>
                                            <dd><xsl:choose>
                                            	<xsl:when test="$target-name != res:assessment-results/@subject">
                                            	    <xsl:value-of select="$target-name"/>
                                            	</xsl:when>
                                            	<xsl:otherwise>
                                            	    <a href="{res:assessment-results/@subject}"><xsl:value-of select="res:assessment-results/@subject"/></a>
                                            	</xsl:otherwise>
                                            </xsl:choose></dd>
                                            <dt>Start:</dt>
                                ...
                                <div class="panel panel-default">
                                    ...
                                    <div class="panel-body">
                                        <dl class="dl-horizontal dl-wide">
                                            ...
                                            <dd><xsl:value-of select="count(//res:base-requirement[(res:status/text() != 'NOT_TESTED' or not($ignore-not-tested-results)) and (res:status/text() != 'NOT_IN_SCOPE' or not($ignore-outofscope-results))])"/></dd>
                                            <xsl:if test="//res:base-requirement/res:status[text() = 'NOT_TESTED'] and not($ignore-not-tested-results)">
	                                            <dt>Requirements Not Tested:</dt>
	                                            <dd><xsl:value-of select="count(//res:base-requirement/res:status[text() = 'NOT_TESTED'])"/></dd>
                                            </xsl:if>
                                            <dt>Requirements Passed:</dt>
                                            <dd><xsl:value-of select="count(//res:base-requirement/res:status[text() = 'PASS' or text() = 'WARNING'])"/> of <xsl:value-of select="count(//res:base-requirement/res:status[text() = 'PASS' or text() = 'FAIL' or text() = 'WARNING'])"/></dd>
                                            <dt>Requirements Failed:</dt>
                                            <dd><xsl:value-of select="count(//res:base-requirement/res:status[text() = 'FAIL'])"/></dd>
                                            <dt>Tests Failed:</dt>
                ...
                <script>
                    ...
                    $('table.filterable').filterTable({ignoreClass: 'collapse'});
                </script>
            </body>
        </html>
    </xsl:template>
            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <table class="filterable table table-condensed">
                                <thead>
                                    <tr><th>Requirement #</th><th>Summary</th><th>Result</th></tr>
<!-- 
                                    <tr>
                                    	<th><input class="filterable" type="text" value=""/></th>
                                    	<th><input class="filterable" type="text" value=""/></th>
                                    	<th><input class="filterable" type="text" value=""/></th>
                                    </tr>
 -->
    ...
    <xsl:template match="res:base-requirement" mode="summary">
        <xsl:if test="(res:status/text() != 'NOT_TESTED' or not($ignore-not-tested-results)) and (res:status/text() != 'NOT_IN_SCOPE' or not($ignore-outofscope-results))">
...
<!-- 
                ...
                <xsl:attribute name="class">derived-detail-toggle visible</xsl:attribute>
 -->                
 ...
 -->            
    ...
    <xsl:template match="res:status" mode="status-label">
        ...
        <xsl:choose>
            ...
            <xsl:when test="text() = 'NOT_APPLICABLE'"><span class="status label label-default label-not-applicable">Not Applicable</span></xsl:when>
                                        ...
                                        <div class="panel-body">
                                            <blockquote>
                                                <p class="req-statement">
                                                	<xsl:choose>
                                                		<xsl:when test="$req-id-key/req:reference/@requirement-fragment">
    														<a href="{$requirements/key('resource-index',$req-id-key/req:reference/@ref)/@href}#{$req-id-key/req:reference/@requirement-fragment}"><xsl:value-of select="$req-id-key/req:statement/text()"/></a>
                                                		</xsl:when>
                                                		<xsl:otherwise>
                                                ...
                                                <footer>Section
                                                	<xsl:choose>
                                                		<xsl:when test="$req-id-key/req:reference/@section-fragment">
    														<a href="{$requirements/key('resource-index',$req-id-key/req:reference/@ref)/@href}#{$req-id-key/req:reference/@section-fragment}"><xsl:value-of select="$req-id-key/req:reference/@section"/></a>
                                                		</xsl:when>
                                                		<xsl:otherwise>
	                                                		<xsl:value-of select="$req-id-key/req:reference/@section"/>
                                                		</xsl:otherwise>
                                                	</xsl:choose>
                                                  of <cite title="{$requirements/key('resource-index',$req-id-key/req:reference/@ref)/@name}"><xsl:value-of select="$requirements/key('resource-index',$req-id-key/req:reference/@ref)/@name"/></cite></footer>
                                            </blockquote>
                                        </div>
                                    </div>
                                </xsl:element>
...
<!-- 
 ...
 -->
    ...
    <xsl:template match="res:test" mode="detail">
        ...
        <tr>
            <td colspan="4">
                <xsl:apply-templates select="document(res:location/@href)" mode="xml-to-html">

	...
	<xsl:template match="/">
		<html>
			<head>
                ...
                <link rel="stylesheet" href="https://gitcdn.github.io/bootstrap-toggle/2.2.2/css/bootstrap-toggle.min.css" integrity="sha384-yBEPaZw444dClEfen526Q6x4nwuzGO6PreKpbRVSLFCci3oYGE5DnD1pNsubCxYW" crossorigin="anonymous"></link>
                <script src="https://gitcdn.github.io/bootstrap-toggle/2.2.2/js/bootstrap-toggle.min.js" integrity="sha384-cd07Jx5KAMCf7qM+DveFKIzHXeCSYUrai+VWCPIXbYL7JraHMFL/IXaCKbLtsxyB" crossorigin="anonymous"></script>

                <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-table/1.11.1/bootstrap-table.min.css" integrity="sha384-5nsQ1S/tkV3k79V0HFUsh6FgjFOy7J6Y7zOcNtQRswfMabDb4WrPwSfNBD3fkGBV" crossorigin="anonymous"></link>
                <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-table/1.11.1/bootstrap-table.min.js" integrity="sha384-cKm/6JlIlt9zZV/g+/NqAgMrygFeVtQsI+CWNQtldXF33rCqZYtVL/QH6UTDmQAn" crossorigin="anonymous"></script>
					...
					<div class="card">
						...
						<div class="card-block ml-2 mb-2">
                            <div class="checkbox m-2">
                                ...
                                <input id="filter-switch" data-toggle="toggle" data-on="Failed Only"
                                    data-off="All Results" type="checkbox" />
                            </div>
	...
	<xsl:template match="res:status" mode="status-label">
		...
		<xsl:choose>
			<xsl:when test="text() = 'PASS'">
				<span class="border border-success rounded bg-success text-white status p-1">Pass</span>
			</xsl:when>
			<xsl:when test="text() = 'WARNING'">
				<span class="border border-warning rounded bg-warning text-white status p-1">Warning</span>
			</xsl:when>
			<xsl:when test="text() = 'FAIL'">
				<span class="border border-danger rounded bg-danger text-white status p-1">Fail</span>
			</xsl:when>
            <xsl:when test="text() = 'NOT_IN_SCOPE'">
                <span class="border border-muted rounded bg-muted text-black status p-1">Not In Scope</span>
            </xsl:when>
			<xsl:when test="text() = 'NOT_TESTED'">
				<span class="border border-warning rounded bg-muted text-black status p-1">Not Tested</span>
			</xsl:when>
			<xsl:when test="text() = 'NOT_APPLICABLE'">
				<span class="border border-success rounded bg-muted text-black status p-1">Not Applicable</span>
			</xsl:when>
			<xsl:when test="text() = 'INFORMATIONAL'">
				<span class="border border-info rounded bg-info text-white status p-1">Informational</span>
			</xsl:when>
			<xsl:otherwise>
			    ...
			    <div class="card-header bg-secondary text-white">
					<div class="row">
					    <div class="col-sm-2 align-middle">
					        <h2 class="align-middle mb-0"><xsl:value-of select="$current-req" /></h2>
					    </div>
					    <div class="col-sm-8 align-middle">
					        <p class="align-middle mb-0"><xsl:value-of select="$req-id-key/req:summary/text()" /></p>
					    </div>
                        <div class="col-sm-2 align-middle">
                            <p  class="align-middle text-right mb-0"><xsl:apply-templates select="res:status"
                                mode="status-label" /></p>
                        </div>
					</div>
			    </div>
		...
		<xsl:if
			...
			<div id="{$current-req}" class="card mb-1">
                ...
                <div class="card-body">
	                ...
	                <div class="card-block">
	                    <blockquote class="card-blockquote">
                            <p class="req-statement"><xsl:value-of
 							    select="$req-id-key/req:statement/text()"/></p>
 					    </blockquote>
 					</div>

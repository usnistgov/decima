...
<!-- OVERVIEW : iso_dsdl_include.xsl This is an inclusion preprocessor for 
	the non-smart text inclusions of ISO DSDL. It handles <relax:extRef> for 
	ISO RELAX NG <sch:include> for ISO Schematron and Schematron 1.n <sch:extends> 
	for 2009 draft ISO Schematron <xi:xinclude> simple W3C XIncludes for ISO 
	NVRL and DSRL <crdl:ref> for draft ISO CRDL <dtll:include> for draft ISO 
	DTLL <* @xlink:href> for simple W3C XLink 1.1 embedded links This should 
	be the first in any chain of processing. It only requires XSLT 1. Each kind 
	of inclusion can be turned off (or on) on the command line. Ids in fragment 
	identifiers or xpointers will be sought in the following order: * @xml:id 
	* id() for typed schemas (e.g. from DTD) [NOTE: XInclude does not support 
	this] * untyped @id The proposed behaviour for the update to ISO Schematron 
	has been implemented. If an include points to an element with the same name 
	as the parent, then that element's contents will be included. This supports 
	the merge style of inclusion. When an inclusion is made, it is preceded by 
	a PI with target DSDL_INCLUDE_START and the href and closed by a PI with 
	target DSDL_INCLUDE_START and the href. This is to allow better location 
	of problems, though only to the file level. Limitations: * No rebasing: relative 
	paths will be interpreted based on the initial document's path, not the including 
	document. (Severe limitation!) * No checking for circular references * Not 
	full xpointers: only ID matching * <relax:include> not implemented * XInclude 
	handling of xml:base and xml:lang not implemented -->
<!-- VERSION INFORMATION 2010-04-21 * Add basic syntax checks on XPaths belonging 
	to Schematron elements only * Unlocalized messages are put out using xsl:message. 
	The intent is to allow * problems to be caught at compile time. 2009-02-25 
	* Update DSDL namespace to use schematron.com * Tested with SAXON9, Xalan 
	2.7.1, IE7, * IE does not like multiple variables in same template with same 
	name: rename. 2008-09-18 * Remove new behaviour for include, because it conflicts 
	with existing usage [KH] * Add extends[@href] element with that merge functionality 
	* Generate PIs to notate source of inclusions for potential better diagnostics 
	2008-09-16 * Fix for XSLT1 2008-08-28 * New behaviour for schematron includes: 
	if the pointed to element is the same as the current, include the children. 
	2008-08-20 * Fix bug: in XSLT1 cannot do $document/id('x') but need to use 
	for-each 2008-08-04 * Add support for inclusions in old namespace 2008-08-03 
	* Fix wrong param name include-relaxng & include-crdl (KH, PH) * Allow inclusion 
	of XSLT and XHTML (KH) * Fix inclusion of fragments (KH) 2008-07-25 * Add 
	selectable input parameter 2008-07-24 * RJ New -->
<!-- LEGAL INFORMATION Copyright (c) 2008 Rick Jelliffe This software is 
	provided 'as-is', without any express or implied warranty. In no event will 
	the authors be held liable for any damages arising from the use of this software. 
	Permission is granted to anyone to use this software for any purpose, including 
	commercial applications, and to alter it and redistribute it freely, subject 
	to the following restrictions: 1. The origin of this software must not be 
...
<xslt:stylesheet version="1.0"
	xmlns:xslt="http://www.w3.org/1999/XSL/Transform" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:iso="http://purl.oclc.org/dsdl/schematron" xmlns:nvdl="http://purl.oclc.org/dsdl/nvdl"
	xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:schold="http://www.ascc.net/xml/schematron"
	xmlns:crdl="http://purl.oclc.org/dsdl/crepdl/ns/structure/1.0"
	xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:dtll="http://www.jenitennison.com/datatypes"
	xmlns:dsdl="http://www.schematron.com/namespace/dsdl" xmlns:relax="http://relaxng.org/ns/structure/1.0"
	xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sch-check="http://www.schematron.com/namespace/sch-check">
	<!-- Note: The URL for the dsdl namespace is not official -->


	<xsl:param name="include-schematron">
		true
	</xsl:param>
	<xsl:param name="include-crdl">
		true
	</xsl:param>
	<xsl:param name="include-xinclude">
		true
	</xsl:param>
	<xsl:param name="include-dtll">
		true
	</xsl:param>
	<xsl:param name="include-relaxng">
		true
	</xsl:param>
	<xsl:param name="include-xlink">
		true
	</xsl:param>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:choose>
					...
					<!-- this case is when there is in embedded schema in the same document 
						elsewhere -->
					...
					<xsl:when test="string-length( $fragment-id ) &gt; 0">
						...
						<!-- use a for-each so that the id() function works correctly on the 
							external document -->
              ...
              | $theDocument_1//*[@id= $fragment-id ]" />
							...
							<xsl:apply-templates select=" $theFragment_1[1]"
								mode="dsdl:go" />
						</xsl:for-each>
					</xsl:when>
	...
	<!-- Add experimental support for simple containers like /xxx:xxx/iso:pattern 
		to allow better includes -->
			...
			<xsl:otherwise>
				...
				<xsl:choose>
              	 ...
              	 | //iso:*[@id= $fragment-id ]" />
					...
					<!-- case where there is a fragment in another document (should be an 
						iso: element) -->
					<!-- There are three cases for includes with fragment: 0) No href file 
						or no matching id - error! 1) REMOVED 2) The linked-to element is sch:schema 
						however the parent of the include is not a schema. In this case, it is an 
						error. (Actually, it should be an error for other kinds of containment problems, 
						but we won't check for them in this version.) 3) Otherwise, include the pointed-to 
						element -->
					...
					<xsl:otherwise>
						...
						<!-- There are three cases for includes: 0) No text specified- error! 
							1) REMOVED 2) The linked-to element is sch:schema however the parent of the 
							include is not a schema. In this case, it is an error. (Actually, it should 
							be an error for other kinds of containment problems, but we won't check for 
							them in this version.) 3) Otherwise, include the pointed-to element -->
						<xsl:choose>
							<!-- case 0 -->
							...
							<xsl:when
								...
								<xsl:message>
									...
									fragments, not a whole
									schema
								</xsl:message>
							</xsl:when>

							<!-- If this were XLST 2, we could use if ($theFragment) then $theFragment 
								else $theContainedFragments here (thanks to KN) -->
							<!-- case 3 -->
							<xsl:otherwise>
								<xsl:apply-templates select="$theFragment_2 "
									mode="dsdl:go" />
	...
	<!-- Basically, it adds the children of the selected element, not the element 
		itself. -->
				...
				<xsl:choose>
              	 ...
              	 | //iso:*[@id= $fragment-id ]/*" />
					...
					<!-- case where there is a fragment in another document (should be an 
						iso: element) -->
					<!-- There are three cases for includes with fragment: 0) No href file 
						or no matching id - error! 1) REMOVED 2) REMOVED 3) Otherwise, include the 
						pointed-to element -->
					...
					<xsl:otherwise>
						...
						<!-- There are three cases for includes: 0) No text specified- error! 
							1) REMOVED 2) REMOVED 3) Otherwise, include the pointed-to element -->
						<xsl:choose>
							<!-- case 0 -->
							<xsl:when test="not($theFragment_2) and not ($theContainedFragments)">
								<xsl:message terminate="no">
	...
	<!-- Extend the URI syntax to allow # references -->
	<!-- Add experimental support for simple containers like /xxx:xxx/schold:pattern 
		to allow better includes -->
					...
					<xsl:otherwise>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								...
								<xsl:apply-templates select="  $theContainedFragments "
									mode="dsdl:go" />
							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
              ...
              | $theDocument_1//*[@id= current()/@xpointer  ]" />
						<!-- removed | $theDocument_1/id( @xpointer) because it requires rebasing 
							in XSLT1 and that would mess up the use of current() -->
						...
						<xsl:if test="not($theDocument_1)">
                    	 ...
                    	 or @parseType='xml']/@href,/ )" />
              				...
              				| $theDocument_2//*[@id= current()/xi:fallback[1]/xi:include/@xpointer  ]" />
									<!-- removed | $theDocument_2/id( xi:fallback[1]/xi:include/@xpointer) 
										because it id() would need rebasing in XSLT1 and that would mess up use of 
										current() -->
	...
	<xsl:template name="sch-check:xpath-wf">
		...
		<!-- This does some minimal checks to see if a string is well-formed XPath. 
			It checks 1) String is not empty, 2) equal number of open and close parens 
			3) equal number of left and right square brackets It does not check balancing. 
			It does not check inside string literals in XPaths. If there is no error, 
			empty content is returned. If there is an error, it is given as an error 
			message. This is not localized yet. -->
		...
		<xsl:choose>
			...
			<!-- not implemented yet <xsl:when test=" count () mod 2 = 1" >XPath syntax 
				error. Odd number of apostrophe characters. Suggestion: check string termination 
				and delimiting. <xsl:value-of select=" normalize-space($string)"/></xsl:when> 
				<xsl:when test=" count ( ) mod 2 = 1" >XPath syntax error. Odd number of 
				quote characters. Suggestion: check string termination and delimiting. <xsl:value-of 
				select=" normalize-space($string)"/></xsl:when> -->
	...
	<xsl:template name="sch-check:strip-strings">
		...
		<!-- mode 0 = outside string mode 1 = in double quote string mode 2 = in 
			single quote string -->
		<xsl:choose>

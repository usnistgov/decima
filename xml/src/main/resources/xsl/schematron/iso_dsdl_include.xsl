...
<xslt:stylesheet version="1.0"
	...
	<xslt:template match="iso:value-of[@select]"
		mode="dsdl:go">
		<xsl:call-template name="sch-check:xpath-wf-message">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
               ...
               | $theDocument_1//*[@id= $fragment-id ]" />
							...
							<xsl:apply-templates
								select=" $theFragment_1 " mode="dsdl:go" />
						</xsl:for-each>
					</xsl:when>
	...
	<xsl:template name="sch-check:xpath-wf">
		...
		<xsl:choose>
			<xsl:when
				test="string-length( normalize-space($string)) = 0">

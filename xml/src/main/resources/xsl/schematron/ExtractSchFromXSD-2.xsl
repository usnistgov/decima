...
<!-- based on an original transform by Eddie Robertsson 2001/04/21 fn: added 
	support for included schemas 2001/06/27 er: changed XMl Schema prefix from 
	xsd: to xs: and changed to the Rec namespace 2010/04/14 rj: Update for ISO 
	Schematron using xslt2 -->
<xsl:transform version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sch="http://purl.oclc.org/dsdl/schematron"
	...
	<xsl:template name="gatherSchema">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<!-- we have the complete set of included schemas, so now let's output 

...
public class SaxonXPathFactoryTest {
  ...
  public void test() {
    ...
    XPathExpression<Element> elementExpression = factory.compile("//ns:*[@xml:base]",
        Filters.element(), Collections.emptyMap(), Namespace.XML_NAMESPACE, ns);
    List<Element> result = elementExpression.evaluate(document);
    Assert.assertEquals(Collections.singletonList(child1), result);

    XPathExpression<Attribute> attributeExpression
        = factory.compile("//@xml:base[$test='test-var']", Filters.attribute(),
            Collections.singletonMap("test", "test-var"), Namespace.XML_NAMESPACE, ns);

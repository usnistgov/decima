...
public class CLIParser {
    ...
    protected void initializeStandardOptions() {
        ...
        Option reportFile = Option.builder(OPTION_VALIDATION_REPORT_FILE)
                .desc("The validation HTML report file location (default: " + DEFAULT_VALIDATION_REPORT_FILE + ")")
                .hasArg().argName("FILE").build();

        Option versionOption = Option.builder(OPTION_VERSION).desc("Display the version of the tool").build();

        Option helpOption
                = Option.builder(OPTION_HELP).longOpt("help").desc("Display the available cli arguments").build();

        getOptions().addOptionGroup(outputControlGroup).addOption(resultFile).addOption(reportFile)
                .addOption(versionOption).addOption(helpOption);
    }
